...
# Rana boylii range
poly<- st_read("data/shps/RABO_Potential_Range_CAandOR.shp", quiet=T)
poly<- st_transform(poly, crs=4326)

# All RABO Points
#rbpts<- st_read("data/shps/RABO_All_Records_thru_2010_full_v4.shp", quiet = T)
#rbpts <- st_transform(rbpts, crs=4326)

# Hydrology
h8 <- st_read("data/shps/hydrology/HUC8_named_westcoast.shp", quiet=T)

# this is all conversions to make a nice named shapefile of H8's for OR and CA:

# h8_OR <- st_read("data/shps/hydrology/h8_OR_named.shp", quiet = T, stringsAsFactors = F) 
# h8_OR <- st_transform(h8_OR, crs = 4326)
# h8_CA <- st_read("data/shps/hydrology/wbdhu8_a_ca.shp", quiet = T,stringsAsFactors = F) %>% 
#   rename(HUC_8=HUC8)
# h8_CA <- st_transform(h8_CA, crs = 4326)
# h8_CA <- select(h8_CA, HUC_8:Name,AreaAcres, AreaSqKm) %>% rename(HU_8_NAME=Name)

# h8<- st_read("data/shps/hydrology/HUC_8_WESTCOAST.shp", quiet = T, stringsAsFactors = F) %>% 
  # select(HUC_8, HU_8_NAME, ACRES)
...
# h8_named <- select(h8_named, HUC_8, HU_8_NAME.x, HU_8_NAME.y, AreaAcres, AreaSqKm, SHAPE_Area)
# h8_named$HU_8_NAME <- ifelse(is.na(h8_named$HU_8_NAME.x), h8_named$HU_8_NAME.y, h8_named$HU_8_NAME.x)
# h8_named <- select(h8_named, HUC_8, HU_8_NAME, AreaAcres, AreaSqKm, SHAPE_Area)
# st_write(h8_named, "data/shps/hydrology/HUC8_named_westcoast.shp")
...
MAP<-leaflet() %>%
  ...
  # Historical Range
  addPolygons(data = poly, group = "RABO HistRange", weight = 4,stroke = TRUE,
  ...
  addLayersControl(
    ...
    overlayGroups = c("Rana Samples", "Mine Pits", "CA Mines", 
                      "RABO HistRange", "HUC8", 
